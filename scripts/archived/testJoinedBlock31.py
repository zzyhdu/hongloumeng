"""Test AI on actual block 31 text from flush."""
import sys
sys.path.insert(0, '.')

from scripts.parseZhipingJson import is_poetry_text

# Block 31 spans joined together (as they would be in flush_block)
test_text = ("一日，早又中秋佳节。士隐家宴已毕，乃又另具一席于书房，却自己步月至庙中来邀雨村。"
             "原来雨村自那日见了甄家之婢曾回头顾他两次，自为是个知己，便时刻放在心上。"
             "今又正值中秋，不免对月有怀，因而口占五言一律云：")

print("Block 31 text (joined spans):")
print(test_text)
...
for i, line in enumerate(result['poem_lines']):
    print(f"  {i}: {line}")
print(f"remaining_text: {result['remaining_text'][:80] if result['remaining_text'] else '(empty)'}...")
